...
from qualiteacher.archs import build_network
from basicsr.losses import build_loss
from basicsr.metrics import calculate_metric
from basicsr.utils import get_root_logger, imwrite, tensor2img
from basicsr.utils.registry import MODEL_REGISTRY
from basicsr.models.base_model import BaseModel
from qualiteacher.models import lr_scheduler as lr_scheduler
...
class SRModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if self.ema_decay > 0:
            ...
            if load_path is not None:
                ...
                try:
                    self.load_network(self.net_g_ema, load_path, strict_load, 'params_ema')
                except KeyError:
                    logger.info(f'params_ema not found, loading EMA with {param_key}')
                    self.load_network(self.net_g_ema, load_path, strict_load, param_key)
            else:
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        print(f"Verification begins - Dataset size: {len(dataloader)}")
        with torch.no_grad():
            ...
            print("The index initialization was successful.")

            for idx, val_data in enumerate(dataloader):
                print(f"Processing the {idx}th image")
                #if idx >= 5:
                     #break
