...
class Upsample(nn.Module):
    def __init__(self, n_feat):
        ...
        self.body = nn.Sequential(nn.Conv2d(n_feat, n_feat*2, kernel_size=3, stride=1, padding=1, bias=False),
                                  nn.PixelShuffle(2),
                                  nn.Conv2d(n_feat // 2, n_feat // 2, 3, 1, 1))
        #Dirac init: identity at start, won't break pretrained weights
        nn.init.dirac_(self.body[2].weight)
...
class MST(nn.Module):
    def __init__(self, in_dim=30, out_dim=30, dim=30, stage=2, num_blocks=[2,4,4], use_score_emb=False):
        ...
        if use_score_emb:
            self.score_embedding = nn.Embedding(8, 120)
    ...
    def forward(self, x, y=None, score=None):
        """
        x: [b,c,h,w]
        return out:[b,c,h,w]
        """
        #if self.use_score_emb:
            #assert score is not None, "use_score_emb=True but score is None! Must pass score parameter."


        # Embedding
        fea = self.embedding(x) if y is None else self.embedding(torch.cat([x, y], dim=1))
        #print(f"After embedding: {fea.shape}")
        # Encoder
        fea_encoder = []
        for idx, (MSAB, FeaDownSample) in enumerate(self.encoder_layers):
            fea = MSAB(fea)
            #print(f"Encoder stage{idx} after MSAB: {fea.shape}")
            if idx == 2 and self.use_score_emb and score is not None:
                B, C, H, W = fea.shape
                # if not hasattr(self, '_feature_size_logged'):
                #     print(f"[FEATURE SIZE CHECK] Encoder stage2: C={C}, H={H}, W={W}")
                #     self._feature_size_logged = True
                    
                score_emb = self.score_embedding(score)
                score_emb = score_emb.view(B, 120, 1, 1).expand(-1, -1, H, W)
                fea = fea + score_emb

                # if not hasattr(self, '_injection_verified'):
                #     print(f"[SCORE INJECTION] Encoder stage2: score={score}")
                #     self._injection_verified = True

            fea_encoder.append(fea)
            fea = FeaDownSample(fea)
            #print(f"Encoder stage{idx} after Downsample: {fea.shape}")
        # Bottleneck
        fea = self.bottleneck(fea)
        #print(f"After Bottleneck: {fea.shape}")


        # Decoder
        for i, (FeaUpSample, Fution, LeWinBlcok) in enumerate(self.decoder_layers):
            fea = FeaUpSample(fea)
            #print(f"Decoder stage{i} after Upsample: {fea.shape}")
            fea = Fution(torch.cat([fea, fea_encoder[self.stage-1-i]], dim=1))
            #print(f"Decoder stage{i} after Fusion: {fea.shape}")
            if i == 0 and self.use_score_emb and score is not None:
                B, C, H, W = fea.shape
                score_emb = self.score_embedding(score)
                score_emb = score_emb.view(B, 120, 1, 1).expand(-1, -1, H, W)
                fea = fea + score_emb

                # if not hasattr(self, '_injection_verified_decoder'):
                #     print(f"[SCORE INJECTION] Decoder stage0: score={score}")
                #     self._injection_verified_decoder = True

            fea = LeWinBlcok(fea)
            #print(f"Decoder stage{i} after MSAB: {fea.shape}")
...
class Basic_block_fix_Plus(nn.Module):
    def __init__(self, last=False, use_score_emb=False):
        super(Basic_block_fix_Plus, self).__init__()

        self.trans_prox = ProximalTransmissionBlock()
        if not last:
            self.trans_net = MST(in_dim=4, out_dim=1, dim=30, stage=3, num_blocks=[1,1,1], use_score_emb=use_score_emb) # ， use_score_emb=use_emb)

        self.dehaze_prox = ProximalDehazingBlock()
        self.dehaze_net = MST(in_dim=4, out_dim=3, dim=30, stage=3, num_blocks=[1,1,1], use_score_emb=use_score_emb) # ， use_score_emb=use_emb)

    def forward(self, img, stage1_trans, stage1_img, stage1_trans_hat, stage1_img_hat, last=False, score=None):
        ## GDM
        stage2_trans_hat = self.trans_prox(img, stage1_img_hat, stage1_trans)
        if not last:
            stage2_trans = self.trans_net(stage2_trans_hat, stage1_img, score=score)
        else:
            stage2_trans = stage2_trans_hat

        stage2_img_hat = self.dehaze_prox(img, stage1_img, stage2_trans_hat)
        stage2_img = self.dehaze_net(stage2_img_hat, stage2_trans_hat, score=score)

        return stage2_trans, stage2_img, stage2_trans_hat, stage2_img_hat

@ARCH_REGISTRY.register()
class CORUN(nn.Module):
    def __init__(self, depth=3, use_score_emb=False):
        super(CORUN, self).__init__()

        self.depth = depth - 2
        self.basic = nn.Sequential(*[Basic_block_fix_Plus(use_score_emb=use_score_emb)for _ in range(self.depth)])#, is_basic=True)for _ in range(self.depth)])
        self.first = Basic_block_fix_Plus(use_score_emb=use_score_emb)#, is_basic=True)
        self.last = Basic_block_fix_Plus(last=True, use_score_emb=use_score_emb)#, is_basic=False)

    def forward(self, img, score=None, debug=False, t_mode=False, finetune=False):
        ...
        stage1_trans, stage1_img, stage1_trans_hat, stage1_img_hat = self.first(img, stage1_trans, stage1_img, stage1_trans_hat, stage1_img_hat, score=score)
        ...
        for i in range(self.depth):
            stage1_trans, stage1_img, stage1_trans_hat, stage1_img_hat = self.basic[i](img, stage1_trans, stage1_img, stage1_trans_hat, stage1_img_hat, score=score)
            res.append(stage1_img)
            if debug:
                disp_tres.append(torch.cat([stage1_trans_hat, stage1_trans], 3))
                ires.append(torch.cat([stage1_img_hat, stage1_img], 3))
        ...
        stage1_trans, stage1_img, stage1_trans_hat, stage1_img_hat = self.last(img, stage1_trans, stage1_img, stage1_trans_hat, stage1_img_hat, last=True, score=score)
        res.append(stage1_img)
        if debug:
            disp_tres.append(torch.cat([stage1_trans_hat, stage1_trans], 3))
            ires.append(torch.cat([stage1_img_hat, stage1_img], 3))
