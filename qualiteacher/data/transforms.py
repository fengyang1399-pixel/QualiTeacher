...
def triple_random_crop(img_gts, img_lqs, img_t, lq_patch_size, scale, gt_path):
    """Paired random crop.
    ...
    if len(img_t) == 1:
        img_t = img_t[0]
    return img_gts, img_lqs, img_t

def paired_resize(img_gts, img_lqs, gt_patch_size):
    # resize numpy image
    img_gts = cv2.resize(img_gts, (gt_patch_size, gt_patch_size),
                             interpolation=cv2.INTER_CUBIC)
    img_lqs = cv2.resize(img_lqs, (gt_patch_size, gt_patch_size),
                             interpolation=cv2.INTER_CUBIC)
    return img_gts, img_lqs
...
def solo_random_crop(img, patch_size, scale):
    """Paired random crop.
