...
from qualiteacher.data.transforms import mod_crop
from qualiteacher.utils import img2tensor, scandir
import random
import os
...
def merge_paths_from_folder(folders, keys, filename_tmpl):
    try:
        input_folder, gt_folder = folders
        ...
        for i in range(len(input_paths)):
            paths.append([osp.join(input_folder, input_paths[i]),np.array(0)])
            only_lq_paths.append([osp.join(input_folder, input_paths[i]),np.array(0)])
        for i in range(len(gt_paths)):
            paths.append([osp.join(gt_folder, gt_paths[i]),np.array(1)])
            only_hq_paths.append([osp.join(gt_folder, gt_paths[i]),np.array(1)])

        random.shuffle(paths)
        random.shuffle(only_lq_paths)
        random.shuffle(only_hq_paths)
    ...
    return paths, only_lq_paths, only_hq_paths


def clip_paths_from_folder(folders, keys, filename_tmpl):
    input_folder, gt_folder = folders
...
def unpaired_paths_from_folder(folder, keys, filename_tmpl):
...
def all_paths_from_folder(folders, keys, filename_tmpl):
    """Generate paired paths from folders.
